...
function getCorsHeaders(request: Request, env: Env): Record<string, string> {
  ...
  const allowedOrigins = new Set([
    "https://floppy.cloud",
    "https://www.floppy.cloud",
  ]);

  const allowOrigin = env.ENVIRONMENT === "production"
    ? (allowedOrigins.has(origin) ? origin : "https://floppy.cloud")
    : "*";
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/api/rooms") {
        ...
        if (!isValidPhrase(phrase)) return new Response("Invalid phrase", { status: 400, headers: corsHeaders });
      ...
      if (request.method === "GET" && url.pathname === "/api/turn") {
        ...
        if (limited) return new Response("Rate limit exceeded", { status: 429, headers: corsHeaders });
        await recordRequest(env, ip, "turn");
